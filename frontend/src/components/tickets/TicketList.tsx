...
export function TicketList({ activeTicketId, viewMode, onViewModeChange, onSelect }: { activeTicketId: string | null, viewMode: "list" | "kanban", onViewModeChange: (mode: "list" | "kanban") => void, onSelect?: (id: string) => void }) {
  ...
  return (
    <div className="flex flex-col h-full bg-surface border-r border-border min-w-0">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center border border-border-subtle rounded-md overflow-hidden bg-background-subtle p-0.5">
            <button 
              ...
              className={`p-1 transition-colors rounded-[4px] ${viewMode === 'list' ? 'bg-surface shadow-sm text-foreground' : 'text-foreground-muted hover:text-foreground'}`}
              title="List View"
            >
            ...
            <button 
              ...
              className={`p-1 transition-colors rounded-[4px] ${viewMode === 'kanban' ? 'bg-surface shadow-sm text-foreground' : 'text-foreground-muted hover:text-foreground'}`}
              title="Kanban View"
            >
      ...
      <div className="flex-1 overflow-auto bg-background relative min-w-0">
        {isLoading ? (
            ...
            {tickets.length === 0 ? (
              ...
              tickets.map((ticket) => (
                <div 
                  ...
                  className={`flex flex-col p-4 border-b border-border-subtle cursor-pointer transition-colors hover:bg-surface ${activeTicketId === ticket.id ? 'bg-primary/5 border-l-2 border-l-primary' : 'border-l-2 border-l-transparent'}`}
                >
            ...
            {kanbanColumns.map(status => (
              ...
              >
                <div className="flex items-center gap-2 p-1.5 mb-1">
                  ...
                  <span className="ml-auto text-[10px] text-foreground-muted font-medium bg-background-subtle px-1.5 py-0.5 rounded-sm border border-border-subtle">
                ...
                <div className="flex flex-col gap-2 overflow-y-auto pr-1 pb-1 flex-1">
                  {ticketsByStatus[status]?.map(ticket => (
                    <div 
                      ...
                      onClick={() => router.push(`/tickets/${ticket.id}`)}
                      className={`bg-background p-3 rounded-md border border-border-subtle shadow-sm cursor-grab active:cursor-grabbing hover:border-primary/50 transition-colors ${activeTicketId === ticket.id ? 'ring-1 ring-primary' : ''}`}
                    >
                      <div className="flex justify-between items-start mb-2">
                        <span className="text-[10px] font-mono text-foreground-muted">{ticket.ticketNumber}</span>
