    ...
    <Popover>
      ...
      <PopoverContent className="w-80 p-0" align="end">
        <div className="flex items-center justify-between px-4 py-3 border-b border-border">
          ...
          {count > 0 && <span className="text-xs text-foreground-muted tabular-nums">{count} unread</span>}
        </div>
            ...
            notifications.map((n: any) => (
              ...
              >
                <div className="flex flex-col gap-1">
                  ...
                  <span className="text-[10px] text-foreground-subtle mt-1 tabular-nums">{new Date(n.createdAt).toLocaleTimeString()}</span>
                </div>
              </Link>
            ))
          )}
