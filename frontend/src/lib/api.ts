...
async function fetchWithInterceptor(url: string, options: RequestInit = {}) {
  // Always use relative path in client to leverage Next.js proxy/rewrites
  const baseUrl = typeof window !== 'undefined' ? '/api/v1' : (process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5001/api/v1');
